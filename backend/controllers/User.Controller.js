...
const loginUser = async (req, res) => {
  ...
  {
    User.findOne({ Email })
    .then(user => res.status(200).json(`the email is ${user.Email} and the password is ${user.Password} and the token is ${token}`))
...
const registerUser = async (req, res) => {
    ...
    // const newUser = new User({ Email:Email, Password: hashedPass});
    // res.json(newUser);
